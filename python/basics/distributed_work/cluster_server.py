...
shared_directory = "." + os.sep + "bin" + os.sep
port_number = 10000
batch_time = 10 # number of seconds per batch job
...
class ClusterServer:
    ...
    def register_with_disk(self):
        ...
        if os.path.exists(shared_directory) == False:
    ...
    def handle_register(self, request, conn):
        ...
        if current_op.has_more_work():
            ...
            self.send_command(key, {
                "op": "run_op", "op_name": current_op.op + "_setup"})
    ...
    def handle_distribute(self, request, conn):
        ...
        if current_op.has_completed():
            ...
            for k in self.worker_nodes:
                ...
                self.send_command(k, {"op": "run_op", "op_name": op_name})
