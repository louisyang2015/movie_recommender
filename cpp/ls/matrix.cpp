...
{
    if (use_sorted_rows && sorted_rows == nullptr)
        throw "Software told to use \"sorted rows\" without initializing sorted_rows";
    ...
    {
        int row_number = i;
        if (use_sorted_rows) row_number = sorted_rows[i];
...
void SparseMatrix::transpose_multiply(int thread_id, ColVector* x, 
    ColVector* result, bool use_sorted_rows)
{
    if (use_sorted_rows && sorted_rows == nullptr)
        throw "Software told to use \"sorted rows\" without initializing sorted_rows";
...
void SparseMatrix::count_rows(int thread_id, int* bin_counts, int bin_bit_shift)
{
    // The final bin index is (cols - 1) bit shifted
    int last_bin_index = (cols - 1) >> bin_bit_shift;

    // initialize bin_counts to zero
    for (int i = 0; i <= last_bin_index; i++)
        bin_counts[i] = 0;
    ...
    for (int i = start; i < end; i++) // i-th row
    {
        // get the column associated with this row
        int col = col_indices[row_indices[i]];
        int bin_index = col >> bin_bit_shift;
...
void SparseMatrix::compute_sorted_rows(int* bin_counts, int bin_bit_shift)
{
    this->bin_bit_shift = bin_bit_shift;

    // convert bin_counts to bin_indices
    int* bin_indices = bin_counts;
    int last_bin_index = (cols - 1) >> bin_bit_shift;
     
    int prev_bin_value = bin_indices[0];
    bin_indices[0] = 0;

    for (int i = 1; i <= last_bin_index; i++)
    {
        int current_bin_value = bin_indices[i];
        bin_indices[i] = bin_indices[i - 1] + prev_bin_value;
        
        prev_bin_value = current_bin_value; // for next iteration
    }

    // allocate sorted_rows and initialize
    sorted_rows = new int[rows];
    for (int i = 0; i < rows; i++) // i-th row
    {
        int col = col_indices[row_indices[i]];
        int index = col >> bin_bit_shift;
        int sorted_rows_index = bin_indices[index];

        // put item into bin location, then increase this location index
        sorted_rows[sorted_rows_index] = i;
        bin_indices[index]++;
    }
}
    ...
    bool use_sorted_rows)
{
    ...
    for (int i = 0; i < thread_count; i++)
        t[i] = new thread([m, i, x, result, use_sorted_rows]()
                          {m->multiply(i, x, result, use_sorted_rows); });
    ...
    ColVector* result, bool use_sorted_rows)
{
    ...
    for (int i = 0; i < thread_count; i++)
        t[i] = new thread([m, i, x, col_vectors, use_sorted_rows]()
            {m->transpose_multiply(i, x, col_vectors[i], use_sorted_rows); });
...
{
    // allocate bins count arrays
    int last_bin_index = (m->num_cols() - 1) >> bin_bit_shift;
    int bin_length = last_bin_index + 1;

    int** bin_counts = new int*[thread_count];
    for (int i = 0; i < thread_count; i++)
        bin_counts[i] = new int[bin_length];

    ArrayArray<int> bin_counts_wrapper(bin_counts, thread_count);

    // count number of bins
    thread** t = new thread*[thread_count];

    for (int i = 0; i < thread_count; i++)
        t[i] = new thread([m, i, bin_counts, bin_bit_shift]()
                          {m->count_rows(i, bin_counts[i], bin_bit_shift); });

    wait_and_delete_threads(t);

    // merge bin counts and initialize "sorted_rows"
    Array<int> final_bin_count(bin_length);
    for (int i = 0; i < final_bin_count.length(); i++)
    {
        int sum = 0;
        for (int j = 0; j < thread_count; j++)
            sum += bin_counts[j][i];
        final_bin_count[i] = sum;
    }

    m->compute_sorted_rows(final_bin_count.get_ptr(), bin_bit_shift);
}
